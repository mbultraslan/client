...
//        $config['max_height'] = '768';
    ...
    {
        $this->db->select('tbl_user_role.user_id', FALSE);
        $this->db->select('tbl_users.*', FALSE);
        $this->db->from('tbl_user_role');
        $this->db->join('tbl_users', 'tbl_users.user_id = tbl_user_role.user_id', 'left');
    ...
    function task_spent_time_by_id($id, $ptoject = null)
    {
        if (!empty($ptoject)) {
            $where = 'project_id = ' . $id;
        } else {
